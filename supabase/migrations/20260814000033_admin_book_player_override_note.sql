-- ============================================================================
-- Cosmetic-only follow-up to 20260813000032 (gender becomes display-only).
--
-- p_override / v_committed_gender / v_mismatch / the `overridden` response
-- field are UNCHANGED — same computation, same signature, same values a
-- caller sees. This migration adds nothing but an inline comment marking them
-- inert, so a future reader of the function source (not just the migration
-- history) sees it without having to dig up 20260813000032. The RPC signature
-- stays exactly as-is deliberately: the admin app still passes `p_override`
-- on every call, and changing the parameter list is a separate, bigger call
-- (a client contract change) that hasn't been made.
-- ============================================================================
